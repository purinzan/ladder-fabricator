...
| logic | はい | 下記の条件構造 |
| output | はい | device必須、typeは省略可でcoilのみ |
...
- 文書のタイトル・描画サイズ
- 回路ごとの `nodes`（接点・コイル・分岐/合流・母線）
- `connections`（ID、送信元outポート、接続先inポート）
- `output_condition`（出力IDと導出した条件構造）
...
論理的な運転条件の妥当性、PLCの実行、保持状態、現在値の判定ではありません。
