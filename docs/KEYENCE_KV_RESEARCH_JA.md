...
| KV-X500シリーズ ユーザーズマニュアル | KV-X500 | 調査環境へ公式PDFを取得済み（Git対象外） | CPU・デバイス・実行仕様 |
...
1. **完了:** 既存JSON v1を読み続けられるv2パーサを追加した。
2. **完了:** 中立的な接点極性・エッジ・出力意味へ正規化した。
3. **完了:** `melsec-iq-f`と`keyence-kv-x`のメーカー別プロファイルを追加した。
4. **基本範囲完了:** KEYENCEのR/MR/LR/CR/T/C/DM/EM/FM/ZF/W/TMを検証し、
   `@`ローカル指定、R/MR/LRのビット番号、B/Wの16進表記を実装した。
5. **基本範囲完了:** RES、DIFU/DIFD、CON、立ち下がり接点をSVGと構造JSONへ反映した。
6. エッジb接点、TMR/TMH/TMS、C/OUTC/UDCを中間形式へ追加する。
7. CALL/SBN/RET、MC/MCR、FBを範囲・入口・出口付きの構造として追加する。
8. KV STUDIOの画面を追加計測し、現在のKEYENCE描画テーマを精密化する。
9. 現行KV-Xコマンドリファレンスを取り込み、全命令、型サフィックス、CPU制限を登録する。
10. 合成回路でSVGと構造JSONを検証し、実機投入形式への変換は別機能として扱う。
